...
# plt_ptch_list <- 1:length(area_nm_list) %>% 
  purrr::map(function(xx){
    scenario_cnstrnt_map_fn(xx) +
    geom_sf(
        data = cnstrnt_class_patches %>%
          dplyr::filter(
            area_name == area_nm_list[xx]
            & patch_area_rank == 1
            & cnstrnt_class != "med. constraint"
          ) %>%
          sf::st_transform(crs=plt_crs)
        , aes(color = cnstrnt_class, fill = NA), lwd = 1.4
        , inherit.aes = F
        , alpha = 0
      ) +
    scale_color_manual(
...
scenario_cnstrnt_map_fn(9) +
  geom_sf(
      data = cnstrnt_class_patches %>%
        ...
        sf::st_transform(crs=plt_crs)
      , aes(color = cnstrnt_class, fill = NA), lwd = 2
      , inherit.aes = F
      , alpha = 0
    ) +
  geom_sf(
  ...
  scale_color_manual(values = cols_temp) +
  guides(
    color = "none"
    , fill = guide_legend(reverse = T, override.aes = list(alpha = 0.9, linewidth = 0))
  )
