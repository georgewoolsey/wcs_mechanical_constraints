...
wf_landscapes <- sf::read_sf("../data/Wildfire_Crisis_Strategy_Landscapes/Wildfire_Crisis_Strategy_Landscapes_(Feature_Layer).shp") %>% 
  ...
  dplyr::left_join(
    # readr::read_csv("../data/wildfirepriority_WCS202306/area_label_placement.csv")
...
constrained_by_scnro_ls <-
  # list.files("../data/wildfirepriority_WCS202306/landscapes/",pattern = "\\.csv$") %>%
  list.files("../data/wildfirepriority/landscapes/",pattern = "\\.csv$") %>%
  purrr::keep(stringr::str_starts(.,"wfpriority_all_sc")) %>% 
  purrr::map(function(x){
    readr::read_csv(
      # paste0("../data/wildfirepriority_WCS202306/landscapes/",x)
...
# export
ggplot2::ggsave(
    filename = paste0("../data/plt_cnstrnt_avail.png")
    , plot = ggplot2::last_plot()
    , width = 8.5
    , height = 11
    , units = "in"
...
ggplot2::ggsave(
    filename = paste0("../data/plt_fpa_cnstrnt_dist.png")
    , plot = ggplot2::last_plot()
    , width = 11
    , height = 8.5
    , units = "in"
...
1:length(plt_ptch_list) %>% 
  purrr::map(function(x){
      ggplot2::ggsave(
          filename = paste0("../data/","plt_map_patch_fpa_",x,".png")
...
agg_cnstrnt_class_patches_temp <- cnstrnt_class_patches %>% 
  sf::st_drop_geometry() %>% 
...
# plot
ggplot(
  data = agg_cnstrnt_class_patches_temp
  , mapping = aes(
    y = scenario_lab
...
# export
ggplot2::ggsave(
    filename = paste0("../data/plt_lrgst_patch_index.png")
    , plot = ggplot2::last_plot()
    , width = 8.5
    , height = 11
    , units = "in"
...
# select area name from area_nm_list
area_num_temp <- 20
...
# export
ggplot2::ggsave(
    filename = paste0("../data/plt_ex_patch_map_bars_combo.png")
    , plot = ggplot2::last_plot()
    , width = 7.7
    , height = 7
    , units = "in"
...
wf_nlcd_area <- readr::read_csv("../data/wildfirepriority_WCS202306/wfpriority_nlcd_area.csv") %>% 
  dplyr::rename_with(tolower) %>% 
  dplyr::left_join(
    wf_landscapes %>% 
...
ggplot2::ggsave(
    filename = paste0("../data/plt_nlcd_area_ls.png")
    , plot = ggplot2::last_plot()
    , width = 11
    , height = 8.5
    , units = "in"
