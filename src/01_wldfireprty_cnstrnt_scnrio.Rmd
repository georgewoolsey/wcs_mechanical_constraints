...
cols_cnstrnt_avail <- c("#7A0403","#466BE3")
cols_obj_r <- c("darkcyan", "gray40", "orangered3")
# fix this one so orange is 3
cols_obj_html <- c("#008b8b", "#bebebe", "#cd3700")
```
...
knitr::include_graphics("https://i.ibb.co/zNH19jp/Presentation1nm.png")
```
...
WCS priority landscape spatial data from the  [USFS Geospatial Data Discovery](https://www.arcgis.com/home/item.html?id=cf9be6c6ec044c338fc6ae27eb586063) site (accessed 2023-09-13).
...
Landscape-level constraint analysis data (i.e., row unique by WCS landscape) was created via this [Google Earth Engine script](https://code.earthengine.google.com/5dc322c9c106500a0d071d6829cb0f6d?noload=true).
...
Fireshed project area constraint analysis data (i.e., row unique by WCS landscape + FPA) was created via this [Google Earth Engine script](https://code.earthengine.google.com/a16626b77d03489cd71ef670ff518d65?noload=true)
...
ggplot2::ggsave(
    filename = paste0("../data/plt_ex_spatial1.jpeg")
    , plot = ggplot2::last_plot()
    , width = 11
    , height = 10
    , units = "in"
...
```{r ls-rdctn-rmn-plt, fig.height=10}
...
```{r ls-rdctn-rmn-plt-wide, fig.height=10}
constrained_by_scnro_ls_long %>% 
  dplyr::mutate(
    constraint = forcats::fct_recode(constraint,"Available"="Total")
    , pct_rdctn_hack = ifelse(constraint=="Available", 1+pct_rdctn, pct_rdctn)
...
```{r, include=FALSE, eval=FALSE}
# pike san isabel to match pods map
# https://cfri.colostate.edu/wp-content/uploads/sites/22/2021/06/CameronPeakFirePODsReport.pdf
# extract file name
    lyr_nms_temp <-
      sf::st_layers(dsn = "c:/Data/usfs/bhef_rxfire_plan/data/S_USA.NFSLandUnit.gdb//S_USA.NFSLandUnit.gdb")$name
    lyr_temp <- lyr_nms_temp[grep("NFSLandUnit", lyr_nms_temp)][1]
  # load in data
    forests_temp <- sf::st_read(
        dsn = "c:/Data/usfs/bhef_rxfire_plan/data/S_USA.NFSLandUnit.gdb//S_USA.NFSLandUnit.gdb"
        , layer = lyr_temp
  ...
  #rename sf geom column
    names(forests_temp)[names(forests_temp)==tolower(attr(forests_temp, "sf_column"))] = "geometry"
    sf::st_geometry(forests_temp) = "geometry"
    
  # transform
    forests_temp <- forests_temp %>% 
      sf::st_transform(crs = transform_crs)

  #filter
  forests_temp <- forests_temp %>% 
      dplyr::filter(
        stringr::str_starts(tolower(nfslandunitname), "pike")
        | stringr::str_starts(tolower(nfslandunitname), "san isabel")
      ) %>% 
      sf::st_union()
##############################################################
##############################################################
...
```{r scenario-maps, fig.height=7, include=F, eval=F}
#####################################################
...
```{r scenario-maps-print, eval=F, include=T}
...
```{r scenario-maps-sad-fn, eval=F, include=F}
# STAMEN MAPS IS DEAD...TRANSITION TO google
...
# bounding box
  bb_temp <- 
    map_firesheds %>% 
    dplyr::filter(
      area_name == area_nm_list[9]
    ) %>% 
    sf::st_union() %>% 
    sf::st_transform(crs=5070) %>% 
    sf::st_buffer(as.numeric(5000)) %>% 
    sf::st_transform(crs=4326) %>% # same as get_map return
    sf::st_bbox()
  # set bbox for get call
  bbox_temp <- c(
  ...
  # get map
  hey_ggmap <- ggmap::get_map(
    location = bbox_temp
    , zoom = 8 # zoom_level
    , scale = "auto"
    , maptype = "satellite" #""terrain-background" # "terrain" # hybrid # satellite
    , source = "google"
    , crop = T
  )
...
**Notes from [Collins et al. 2010](https://scholar.google.com/scholar?cluster=14071982813334795288&hl=en&as_sdt=0,6):**

The larger the individual fuel treatment the greater the potential for tree survival, because of reduced edge effect mortality ([Ritchie et al. 2007](https://scholar.google.com/scholar?cluster=7561962418592858482&hl=en&as_sdt=0,6)). Larger individual treatments have a greater potential to reduce fire behavior and slow fire spread, which ultimately impacts adjacent untreated stands and should enhance suppression opportunities and increase firefighter safety ([Collins et al. 2010](https://scholar.google.com/scholar?cluster=14071982813334795288&hl=en&as_sdt=0,6)). Planning fewer, larger individual treatments across the landscape appears to be a better strategy when human community protection is a primary concern ([Schmidt et al. 2008](https://scholar.google.com/scholar?cluster=9335664484836309865&hl=en&as_sdt=0,6)). [Finney et al. (2007)](https://scholar.google.com/scholar?cluster=11562925721001692199&hl=en&as_sdt=0,6) note that the relative improvement of optimized treatment placement breaks down when larger proportions of the landscape (40–50%) are excluded from treatment because of land-management constraints.
...
#### Example of how patches were done
...
```{r patch-plts-test, include=F, eval=F}
# largest patch with fireshed project areas
scenario_cnstrnt_map_fn(16) +
  geom_sf(
      data = cnstrnt_class_patches %>%
        dplyr::filter(
          area_name == area_nm_list[16]
          & patch_area_rank == 1
          & cnstrnt_class != "med. constraint"
        ) %>%
        sf::st_transform(crs=plt_crs)
...
# largest patch without fireshed project areas
landscape_basemap_fn(16) +
  geom_sf(
      data = cnstrnt_class_patches %>%
        dplyr::filter(
          area_name == area_nm_list[16]
  ...
  scale_fill_manual(values = cols_rdylbu_lt) +
  facet_grid(
    cols = vars(forcats::fct_rev(scenario_lab))
    , labeller = label_wrap_gen(width = 35, multi_line = TRUE)
  ) +
  labs(
    color = "Level of\nConstraint"
...
Planning fewer, larger individual treatments across the landscape appears to be a better strategy when human community protection is a primary concern ([Schmidt et al. 2008](https://scholar.google.com/scholar?cluster=9335664484836309865&hl=en&as_sdt=0,6)). [Finney et al. (2007)](https://scholar.google.com/scholar?cluster=11562925721001692199&hl=en&as_sdt=0,6) note that the relative improvement of optimized treatment placement breaks down when larger proportions of the landscape (40–50%) are excluded from treatment because of land-management constraints.

```{r hi-low-patch-comp, fig.height=10}
...
ggplot(
  ...
  scale_color_manual(values = cols_obj_r) +
...
```{r map-objctv-ls, fig.height=8, warning=F}
# map landscapes shaded by potential to meet 20-40%
plt_fn_temp <- function(scnum=1) {
...
obj_trtbl_temp %>% 
  ...
  dplyr::ungroup() %>% 
  ggplot(
    mapping = aes(y = rnk, x = scenario_lab)
  ) +
    geom_ribbon(mapping = aes(fill = treatment_objective, xmin=-Inf, xmax=Inf)) +
    geom_text(mapping = aes(label = area_name, fontface = "bold"), color = "white") +
    facet_grid(rows = vars(treatment_objective), scales = "free_y", switch = "y") +
    scale_fill_manual(values = c(
      ">20% in\nlow constraint"=cols_obj_r[1]
      ,">20% in\nlow+medium constraint"=cols_obj_r[2]
      , "<20% in\nlow+medium constraint"=cols_obj_r[3])
...
obj_trtbl_temp %>% 
  ...
  dplyr::filter(dplyr::row_number()==1) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(scenario_lab = stringr::str_remove_all(tolower(scenario_lab)," ")) %>% 
  dplyr::select(scenario_lab, area_name, treatment_objective) %>% 
  tidyr::pivot_wider(
    names_from = scenario_lab
    , values_from = c(treatment_objective)
  ) %>% 
  dplyr::arrange(scenario2,scenario1) %>% 
  View()
```
...
cnstrnt_class_patches %>% 
  ...
  dplyr::mutate(
    cnstrnt_class = stringr::str_replace_all(cnstrnt_class," ","_")
  ) %>% 
  tidyr::pivot_wider(
    id_cols = c(area_name,scenario_lab)
    , names_from = cnstrnt_class
    , values_from = max_pct_patch_landscape
    , values_fill = 0
  ) %>% 
  dplyr::relocate(low_constraint, .after = high_constraint) %>% 
  dplyr::mutate(high_is_big = high_constraint>low_constraint) %>% 
  dplyr::arrange(
    desc(scenario_lab)
    , desc(low_constraint)
  ) %>% 
  # dplyr::group_by(scenario_lab) %>% 
  # dplyr::summarise(dplyr::across(c(high_constraint,low_constraint), mean)) %>% 
  View()

### back of napkin calcs for report
# change in overall available acreage across scenarios
constrained_by_scnro_ls %>% 
  ...
  dplyr::arrange(comp_23) %>% 
  View()
### back of napkin calcs for report
# change in lightly constrained acreage across scenarios
constrained_by_scnro_ls_pa %>% 
  ...
  View()
# change in lightly constrained largest patch index
agg_cnstrnt_class_patches %>% 
  dplyr::mutate(scenario_lab = scenario_lab %>% tolower() %>% stringr::str_replace_all(" ", "_")) %>% 
  dplyr::select(area_name, scenario_lab, cnstrnt_class, max_pct_patch_landscape) %>% 
  tidyr::pivot_wider(
    names_from = scenario_lab
    , values_from = c(max_pct_patch_landscape)
  ) %>% 
  dplyr::mutate(
    comp_12 = scenario_2-scenario_1
    , comp_13 = scenario_3-scenario_1
    , comp_23 = scenario_3-scenario_2
  ) %>% 
  dplyr::filter(cnstrnt_class=="low constraint") %>% 
  dplyr::arrange(desc(comp_13)) %>% 
  View()


#####
long_txt <- "priority landscape divided into fireshed project areas. Shadings indicate percentages of the total combined forest and shrubland acres that are available for mechanical treatment: high (81–100% constrained; 0–19% available for mechanical treatment), medium (60–80% constrained; 20–40% available), and low (0–59% constrained; 41–100% available). The largest patch of interconnected fireshed project areas classified as highly and lightly constrained are highlighted."
