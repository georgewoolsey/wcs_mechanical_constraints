...
raster_data_read_fn <- function(row_n = 1) {
 # set up name like in file name
  ...
  files_temp <- list.files("../data/wildfirepriority_WCS202306/images/",pattern = nm_temp,recursive = T) 
  layers_names <- files_temp %>% stringr::word(1,sep = "/")
  rast_list_temp <- files_temp %>%  
    purrr::map(function(x){
      terra::rast(paste0("../data/wildfirepriority_WCS202306/images/",x)) 
    }) %>% 
    # name list objects
...
# landcover_map_fn(row_n = 4, rast_agg_fact = 2)
```
...
roads_map_fn <- function(row_n = 1, rast_agg_fact = 2) {
  # roads sc1
  area_m2_sc1_temp <- rast_list_temp[["sc1"]]$is_roads_distant %>% 
      terra::mask(
        rast_list_temp[["sc1"]]$is_roads_distant %>% 
          terra::clamp(lower = 1, upper = 1, values = FALSE)
      ) %>%
      terra::mask(
        wf_landscapes %>%
          dplyr::filter(area_name==area_nm_list[row_n]) %>% 
          terra::vect() %>%
          terra::project(terra::crs(rast_list_temp[["sc1"]]$is_roads_distant))
      ) %>% 
      terra::freq() %>% 
      dplyr::filter(value==1) %>% 
      dplyr::pull(count) *
        (terra::res(rast_list_temp[["sc1"]]$is_roads_distant) %>% prod())
  # roads sc2
  area_m2_sc2_temp <- rast_list_temp[["sc2"]]$is_roads_distant %>% 
      terra::mask(
        rast_list_temp[["sc2"]]$is_roads_distant %>% 
          terra::clamp(lower = 1, upper = 1, values = FALSE)
      ) %>%
      terra::mask(
        wf_landscapes %>%
          dplyr::filter(area_name==area_nm_list[row_n]) %>% 
          terra::vect() %>%
          terra::project(terra::crs(rast_list_temp[["sc2"]]$is_roads_distant))
      ) %>% 
      terra::freq() %>% 
      dplyr::filter(value==1) %>% 
      dplyr::pull(count) *
        (terra::res(rast_list_temp[["sc2"]]$is_roads_distant) %>% prod())
  # plot forest and shrub cover
  plt <- plt_basemap +
    # scenario 1 roads
    geom_tile(
...
riparian_map_fn <- function(row_n = 1, rast_agg_fact = 2) {
  # riparian sc1
  area_m2_sc1_temp <- rast_list_temp[["sc1"]]$is_riparian_buffer %>% 
      terra::mask(
        rast_list_temp[["sc1"]]$is_riparian_buffer %>% 
          terra::clamp(lower = 1, upper = 1, values = FALSE)
      ) %>%
      terra::mask(
        wf_landscapes %>%
          dplyr::filter(area_name==area_nm_list[row_n]) %>% 
          terra::vect() %>%
          terra::project(terra::crs(rast_list_temp[["sc1"]]$is_riparian_buffer))
      ) %>% 
      terra::freq() %>% 
      dplyr::filter(value==1) %>% 
      dplyr::pull(count) *
        (terra::res(rast_list_temp[["sc1"]]$is_riparian_buffer) %>% prod())
  # riparian sc3
  area_m2_sc3_temp <- rast_list_temp[["sc3"]]$is_riparian_buffer %>% 
      terra::mask(
        rast_list_temp[["sc3"]]$is_riparian_buffer %>% 
          terra::clamp(lower = 1, upper = 1, values = FALSE)
      ) %>%
      terra::mask(
        wf_landscapes %>%
          dplyr::filter(area_name==area_nm_list[row_n]) %>% 
          terra::vect() %>%
          terra::project(terra::crs(rast_list_temp[["sc3"]]$is_riparian_buffer))
      ) %>% 
      terra::freq() %>% 
      dplyr::filter(value==1) %>% 
      dplyr::pull(count) *
        (terra::res(rast_list_temp[["sc3"]]$is_riparian_buffer) %>% prod())
  # plot forest and shrub cover
  plt <- plt_basemap +
    # scenario 1 riparian
    geom_tile(
  ...
  # scenario 3 riparian
    ...
    labs(
      ...
      , caption = paste0(
          ...
          , scales::comma(area_m2_sc3_temp/4046.85642, suffix = "k", scale = 1e-3, accuracy = 1), " acres"
          , " ("
          , scales::comma(area_m2_sc3_temp/10000, suffix = "k", scale = 1e-3, accuracy = 1), " ha"
          , ")"
        )
    ) +
    coord_sf(expand = F) +
...
administrative_map_fn <- function(row_n = 1, rast_agg_fact = 2) {
  # area for caption
  area_m2_temp <- rast_list_temp[["sc1"]]$is_administrative %>% 
    terra::mask(
      rast_list_temp[["sc1"]]$is_administrative %>% 
        terra::clamp(lower = 1, upper = 1, values = FALSE)
    ) %>%
    terra::mask(
      wf_landscapes %>%
        dplyr::filter(area_name==area_nm_list[row_n]) %>% 
        terra::vect() %>%
        terra::project(terra::crs(rast_list_temp[["sc1"]]$is_administrative))
    ) %>% 
    terra::freq() %>% 
    dplyr::filter(value==1) %>% 
    dplyr::pull(count) *
      (terra::res(rast_list_temp[["sc1"]]$is_administrative) %>% prod())
  # plot forest and shrub cover
  plt <- plt_basemap +
    geom_tile(
      data = rast2df_fn(
...
treatable_map_fn <- function(row_n = 1, rast_agg_fact = 2) {
  # area for caption
  # area_m2_temp <- rast_list_temp[["sc1"]]$is_treatable %>% 
  #   terra::mask(
  #     rast_list_temp[["sc1"]]$is_treatable %>% 
  #       terra::clamp(lower = 1, upper = 1, values = FALSE)
  #   ) %>%
  #   terra::mask(
  #     wf_landscapes %>%
  #       dplyr::filter(area_name==area_nm_list[row_n]) %>% 
  #       terra::vect() %>%
  #       terra::project(terra::crs(rast_list_temp[["sc1"]]$is_treatable))
  #   ) %>% 
  #   terra::freq() %>% 
  #   dplyr::filter(value==1) %>% 
  #   dplyr::pull(count) *
  #     (terra::res(rast_list_temp[["sc1"]]$is_treatable) %>% prod())
  # plot forest and shrub cover
  plt <- plt_basemap +
    geom_tile(
    ...
    labs(
      title = "Mechanical Treatment Operability"
...
treatable_map_fn(row_n = 4, rast_agg_fact = 5)
######## export image
ggplot2::ggsave(
